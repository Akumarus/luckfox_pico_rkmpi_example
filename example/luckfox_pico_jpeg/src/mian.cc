...
static void *GetMediaBuffer0(void *arg) {
    ...
    while (!quit && g_saved_frames < MAX_FRAMES_TO_SAVE) {
        ...
        if (s32Ret == RK_SUCCESS) {
            ...
            snprintf(jpeg_path, sizeof(jpeg_path), "%s/image_%03d.jpg", 
                     g_pOutPath, g_saved_frames);
            ...
            if (file) {
                ...
                printf("Saved frame %d/%d: %s (size: %d bytes)\n", 
                       g_saved_frames, MAX_FRAMES_TO_SAVE,
                       jpeg_path, stFrame.pstPack->u32Len);
            } else {
            ...
            // Если сохранили все кадры, выходим
...
int main(int argc, char *argv[]) {
    ...
    // 1. Инициализируем ISP ПЕРЕД VI (как в RTSP примере)
    {
    ...
    // 5. Создаем поток для получения JPEG
    ...
    // Отправляем команду на захват кадров
    for (int i = 0; i < MAX_FRAMES_TO_SAVE && !quit; i++) {
        memset(&stRecvParam, 0, sizeof(VENC_RECV_PIC_PARAM_S));
        stRecvParam.s32RecvPicNum = 1;
    ...
    // Ждем завершения сохранения всех кадров
    while (!quit && g_saved_frames < MAX_FRAMES_TO_SAVE) {
        usleep(100000);
    }
